...
def datestr_and_cycle():
    ...
    if 4 < hour < 10:
        ...
        cycle = '18'
        if hour <= 4:
            datestr = (utcnow - timedelta(days=1)).strftime('%Y%m%d')
...
def ingest_frame(frame,day_diff,datestr,cycle):
    ...
    # parameters = ['2t','tp']
    # filename = '/Users/shedprinter/documents/model-v2/gribs/ecmwf_'+name_frame(frame)+'.grib'
    ...
    # os.system('wget '+idx_url+' -P /Users/shedprinter/documents/model-v2/')
    os.system('curl https://noaa-gefs-pds.s3.amazonaws.com/gefs.'+datestr+'/'+cycle+'/atmos/pgrb2sp25/geavg.t'+cycle+'z.pgrb2s.0p25.f'+name_frame(frame)+'.idx > /Users/shedprinter/documents/model-v2/geavg.t'+cycle+'z.pgrb2s.0p25.f'+name_frame(frame)+'.idx')

    idx_file = '/Users/shedprinter/documents/model-v2/geavg.t'+cycle+'z.pgrb2s.0p25.f'+name_frame(frame)+'.idx'
    ...
    for variable in variables:
        for l in range(len(lines)):
            if lines[l].split(':')[3]+':'+lines[l].split(':')[4]+':'+lines[l].split(':')[5] == variable:
                ...
                if frame!=0:
                    if 'APCP' in variable:
                        os.system('curl -r '+str(start_bytes)+'-'+str(end_bytes)+' '+grib_url+' > /Users/shedprinter/documents/model-v2/gribs/gefs_'+name_frame(frame)+'.grib2')
                    else:
                        os.system('curl -r '+str(start_bytes)+'-'+str(end_bytes)+' '+grib_url+' >> /Users/shedprinter/documents/model-v2/gribs/gefs_'+name_frame(frame)+'.grib2')
                else:
                    os.system('curl -r '+str(start_bytes)+'-'+str(end_bytes)+' '+grib_url+' > /Users/shedprinter/documents/model-v2/gribs/gefs_'+name_frame(frame)+'.grib2')
...
def plot_hourly_snow(ds,f,points,domains):
    ...
    for domain in domains:
        ...
        print('domain slice time: '+str(end-start))
        ...
        # cf = ax.imshow(domain_ds['snow'], extent=[left_lon,right_lon,bottom_lat,top_lat], norm=norm, cmap=colormap)
        ax.coastlines()
        ...
        print('plot setup: '+str(end-start))
        start = time.time()
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/snow/'+str(f)+'_snow.png',dpi=150,bbox_inches='tight')
        end = time.time()
        print('plot save: '+str(end-start))
...
def plot_total_snow(ds,f,points,domains):
    ...
    for domain in domains:
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/total_snow/'+str(f)+'_total_snow.png',dpi=150,bbox_inches='tight')
...
def plot_hourly_precip(ds,f,points,domains):
    ...
    for domain in domains:
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/qpf/'+str(f)+'_qpf.png',dpi=150,bbox_inches='tight')
...
def plot_total_precip(ds,f,points,domains):
    ...
    for domain in domains:
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/total_qpf/'+str(f)+'_total_qpf.png',dpi=150,bbox_inches='tight')
...
def plot_temperature(ds,f,points,domains):
    for domain in domains:
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/temp/'+str(f)+'_temp.png',dpi=150,bbox_inches='tight')
...
def plot_slr(ds,f,method,points,domains):
    for domain in domains:
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/slr/'+str(f)+'_slr.png',dpi=150,bbox_inches='tight')
...
def plot_ptype(ds,f,points,domains):
    ...
    for domain in domains:
        ...
        cf_rain = ax.contourf(domain_ds['lon'],domain_ds['lat'],rain_tp, norm=norm, cmap=ptype_rain())
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/'+str(domain)+'/ptype/'+str(f)+'_ptype.png',dpi=150,bbox_inches='tight')
...
for f in range(0,169,3):
    ...
    if f==0:
        ...
        with open('/Users/shedprinter/documents/points.html', 'r') as myfile:
            text=myfile.read()
    ...
    datestr,cycle = datestr_and_cycle()
    ...
    if f == 0:
        ...
        for domain in domains:
            if not os.path.exists('/Users/shedprinter/documents/model-v2/images/'+domain):
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain)
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/ptype')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/qpf')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/slr')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/temp')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/snow')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/total_snow')
                os.makedirs('/Users/shedprinter/documents/model-v2/images/'+domain+'/total_qpf')
        continue
    ...
    if f == 3:
        chelsa_ds = xr.open_dataset('/Users/shedprinter/documents/11_chelsa0.25.nc')
        chelsa_ds = chelsa_ds.sel(lat=slice(bottom_lat,top_lat),lon=slice(left_lon,right_lon))

    ds = xr.open_dataset('/Users/shedprinter/documents/model-v2/gribs/gefs_'+name_frame(f)+'.grib2')
    prior_ds = xr.open_dataset('/Users/shedprinter/documents/model-v2/gribs/gefs_'+name_frame(f-3)+'.grib2')
    ...
    air_ds = xr.open_dataset('/Users/shedprinter/documents/model-v2/gribs/gefs_air_'+name_frame(f)+'.grib2',engine='cfgrib')
    prior_air_ds = xr.open_dataset('/Users/shedprinter/documents/model-v2/gribs/gefs_air_'+name_frame(f-3)+'.grib2',engine='cfgrib')
    ...
    air_ds = air_ds.interp(latitude=ds['lat'],longitude=ds['lon'])
    ...
    if f == 3:
        elevation_ds = xr.open_dataset('/Users/shedprinter/documents/elevation.nc')
    ...
    ds['tp_maxed'] = xr.where((ds['tp']) > 0.5, 0.5,
                                      ds['tp'])
    ...
    if f == 3:
        total_ds = ds[['tp','snow']]
        total_ds.to_netcdf('/Users/shedprinter/documents/model-v2/total_gribs/gefs_'+name_frame(f)+'.nc')
    else:
        old_total_ds = xr.open_dataset('/Users/shedprinter/documents/model-v2/total_gribs/gefs_'+name_frame(f-3)+'.nc')
        old_total_ds['tp'] = old_total_ds['tp'] + ds['tp']
        old_total_ds['snow'] = old_total_ds['snow'] + ds['snow']
        old_total_ds.to_netcdf('/Users/shedprinter/documents/model-v2/total_gribs/gefs_'+name_frame(f)+'.nc')
    ...
    for n in range(len(points)):
        ...
        point_elevation = int(float(ds.sel(lat=point_lat,lon=point_lon,method='nearest').elevation.values)*3.28084)
        ...
        plt.savefig('/Users/shedprinter/documents/model-v2/images/points/'+point_name+'_all.png',bbox_inches='tight',dpi=150)
